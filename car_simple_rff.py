...
"""
3 RFF instances used (one for each output dimension), todo: for 3rd output (theta) only consider v and delta as input and leave out theta, change seed for each initialization
"""
class RandomFourierFeatures:
    ...
    """
    def __init__(self, input_dim, num_features, lengthscale=1.0, variance=1.0):
...
class CarDynamicsGP:
    ...
    def __init__(self, num_features= 200):
        ...
        self.rff_models = [RandomFourierFeatures(3, num_features) for _ in range(3)]
    ...
    def generate_training_data(self, num_trajectories=50, trajectory_length=100):
        X_train = []
        Y_train = []
        ...
        for _ in range(num_trajectories):
            ...
            for _ in range(trajectory_length):
                ...
                gp_input = np.array([state[2], state[3], control[0]])
                g_noisy = g_true + np.random.normal(0, np.sqrt(self.noise_var), 3)
                X_train.append(gp_input)
    ...
    def train_gp(self, X_train, Y_train):
        ...
        for dim in range(3):
            ...
            A = Phi.T @ Phi + self.noise_var * np.eye(self.num_features)
    ...
    def sample_weights(self, num_samples):
        ...
        for _ in range(num_samples):
            ...
            for dim in range(3):
                ...
                z = np.random.normal(0, 1, self.num_features)
...
class SafeCarMPC:
    ...
    def __init__(self, car_gp, weight_samples, horizon=20, lambda_slack=1e4):
        self.car_gp = car_gp
    ...
    def dynamics_casadi(self, state, control, sample_idx):
        ...
        gp_input = ca.vertcat(theta, v, delta)
        
        g_pred_list = []
        for dim in range(3):
            features = self.car_gp.rff_models[dim].features_casadi(gp_input)
        ...
        return next_state
    # solves mpc after initialization, todo: solver instance lives during all simulation steps -> don't need to copy values, only change first and second value ? 
    def solve_mpc(self, current_state, warm_start_u=None, x_warm = None):
        ...
        if warm_start_u is not None:
             ...
             for n in range(self.num_samples):
                 x1 = self.simulate_sample(current_state, u_shifted[:, 0], n)
                 self.opti.set_initial(self.X_opt[n], x_warm[n])
                 self.opti.set_initial(self.X_opt[n][:, 1], x1)
    ...
    # used in solve_mpc to calculate second state
    def simulate_sample(self, state, control, sample_idx):
        ...
        for dim in range(3):
            features = self.car_gp.rff_models[dim].features(gp_input.reshape(1, -1))
            weights = self.weight_samples[sample_idx][dim]
...
def run_simulation():
    ...
    car_gp = CarDynamicsGP(num_features=20)
    
    print("Generating training data...")
    X_train, Y_train = car_gp.generate_training_data(num_trajectories=20, trajectory_length=50)
    print(f"Training data: {X_train.shape[0]} points")
    
    print("Training GP...")
    car_gp.train_gp(X_train, Y_train)
    ...
    plt.subplot(2, 3, 1)
    plt.plot(states_history[:, 0], states_history[:, 1], 'b-', linewidth=2, label='Actual trajectory')
    
    for obs_x, obs_y in mpc.obstacles:
        circle = plt.Circle((obs_x, obs_y), np.sqrt(5.67), color='red', alpha=0.3)
...
if __name__ == "__main__":
    ...
    print(f"Real-time feasibility: {'✓' if np.mean(times) < 0.06 else '✗'} (target: 60 ms)")
