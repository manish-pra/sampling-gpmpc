...
def __(torch):
    def franke(X, Y):
        term1 = 0.75 * torch.exp(-((9 * X - 2).pow(2) + (9 * Y - 2).pow(2)) / 4)
        term2 = 0.75 * torch.exp(-((9 * X + 1).pow(2)) / 49 - (9 * Y + 1) / 10)
        term3 = 0.5 * torch.exp(-((9 * X - 7).pow(2) + (9 * Y - 3).pow(2)) / 4)
        term4 = 0.2 * torch.exp(-(9 * X - 4).pow(2) - (9 * Y - 7).pow(2))
...
def __(franke, torch):
    ...
    def make_dataset(train_num: int, train_missing: float) -> tuple[Tensor, Tensor]:
        _xv, _yv = torch.meshgrid(
            torch.linspace(0, 1, train_num),
            torch.linspace(0, 1, train_num),
            indexing="ij",
        )
        train_x = torch.cat(
        ...
        if train_missing > 0:
            train_mask = torch.bernoulli(torch.full_like(train_y, train_missing)).to(
                torch.bool
            )
...
def __(franke, torch):
    _xv, _yv = torch.meshgrid(
        torch.linspace(0, 1, 10), torch.linspace(0, 1, 10), indexing="ij"
    )
    train_x = torch.cat(
        (_xv.contiguous().view(_xv.numel(), 1), _yv.contiguous().view(_yv.numel(), 1)),
        dim=1,
    )
    ...
    train_y += 0.05 * torch.randn(
        train_y.size()
    )  # Add noise to both values and gradients
    return train_x, train_y
...
def __(gpytorch, train_x, train_y):
    ...
    likelihood = gpytorch.likelihoods.MultitaskGaussianLikelihood(
        num_tasks=3
    )  # Value + x-derivative + y-derivative
...
def __(gpytorch, likelihood, model, torch, train_x, train_y):
    ...
    def train_gp_model(model, likelihood, train_x, train_y):
        ...
        for i in range(training_iter):
            ...
            loss.backward()
            print(
                "Iter %d/%d - Loss: %.3f   lengthscales: %.3f, %.3f   noise: %.3f"
                % (
                    i + 1,
                    training_iter,
                    loss.item(),
...
def __(
    ...
    with torch.no_grad(), gpytorch.settings.fast_computations(
        ...
        test_x = torch.stack(
            [xv.reshape(n1 * n2, 1), yv.reshape(n1 * n2, 1)], -1
        ).squeeze(1)
    ...
    ax[1, 0].imshow(
        mean[:, 0].detach().numpy().reshape(n1, n2), extent=extent, cmap=cm.jet
    )
    ax[1, 0].set_title("Predicted values")
    ax[1, 1].imshow(
        mean[:, 1].detach().numpy().reshape(n1, n2), extent=extent, cmap=cm.jet
    )
    ax[1, 1].set_title("Predicted x-derivatives")
    ax[1, 2].imshow(
        mean[:, 2].detach().numpy().reshape(n1, n2), extent=extent, cmap=cm.jet
    )
    ax[1, 2].set_title("Predicted y-derivatives")

    ax[2, 0].imshow(
        mean_partial[:, 0].detach().numpy().reshape(n1, n2), extent=extent, cmap=cm.jet
    )
    ax[2, 0].set_title("Predicted values (partial obs)")
    ax[2, 1].imshow(
        mean_partial[:, 1].detach().numpy().reshape(n1, n2), extent=extent, cmap=cm.jet
    )
    ax[2, 1].set_title("Predicted x-derivatives (partial obs)")
    ax[2, 2].imshow(
        mean_partial[:, 2].detach().numpy().reshape(n1, n2), extent=extent, cmap=cm.jet
    )
    ax[2, 2].set_title("Predicted y-derivatives (partial obs)")

    ax[3, 0].imshow(
        mean_pdbg[:, 0].detach().numpy().reshape(n1, n2), extent=extent, cmap=cm.jet
    )
    ax[3, 0].set_title("Predicted values (random masked)")
    ax[3, 1].imshow(
        mean_pdbg[:, 1].detach().numpy().reshape(n1, n2), extent=extent, cmap=cm.jet
    )
    ax[3, 1].set_title("Predicted x-derivatives (random masked)")
    ax[3, 2].imshow(
        mean_pdbg[:, 2].detach().numpy().reshape(n1, n2), extent=extent, cmap=cm.jet
    )
