...
for env in range(0, 1):
    for idx, param in enumerate(param_list):
        if param not in regret_dict:
            regret_dict[param] = []
        for traj_iter in range(0,0+i_max):
            ...
            if idx!=0:
                ...
                cost_l3, solver_status_l3, state_traj_l3 = load_data("data_lap1.pkl")
                close_loop_regret_l3 = np.linalg.norm(state_traj_opt - state_traj_l3[-1], axis=1)
                # close_loop_regret_l2 = np.array(cost_l2) - np.array(cost_opt)
                # merge two regrets
                close_loop_regret = np.concatenate((close_loop_regret, close_loop_regret_l3))
...
# plt.gca().xaxis.set_major_formatter(ticker.FuncFormatter(exp_formatter))
plt.ylim(0.15, 50)
plt.xlim(-0.01, 1550)
# plt.xscale("log")
...
plt.savefig("car_racing.pdf", dpi=600,transparent=True,format="pdf", bbox_inches="tight")