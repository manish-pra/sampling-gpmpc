...
def compute_rkhs_norm(Dyn_gp_X_train, Dyn_gp_Y_train, params, gp_idx=0):
    ...
    lambda_sq = params["agent"]["Dyn_gp_noise"] #+ params["agent"]["Dyn_gp_task_noises"]["val"][0]*params["agent"]["Dyn_gp_task_noises"]["multiplier"]
...
def compute_posterior_norm_diff(Dyn_gp_X_train, Dyn_gp_Y_train, params, gp_idx=0):
    ...
    out = (
        ...
        / (params["agent"]["Dyn_gp_noise"]+ params["agent"]["Dyn_gp_task_noises"]["val"][0]*params["agent"]["Dyn_gp_task_noises"]["multiplier"])
    )
    return out
...
def compute_multi_dim_small_ball_probability(Dyn_gp_X_train, Dyn_gp_Y_train, params, N_grid=8, gp_idx=0):
    ...
    assert not torch.any(torch.isnan(samples))
    sample_diff = samples.squeeze() - model_call.mean.squeeze()

    # plt.plot(X, samples.transpose(0, 1), lw=0.1)
    # plt.savefig("samples.png")
    # sample_diff = sample_diff[:,0,gp_idx,:,0]
    # sample_diff = sample_diff[:,gp_idx,:]
    sample_diff = sample_diff.reshape(total_samples,-1)
    eps = params["agent"]["tight"]["dyn_eps"]
    in_samples = torch.logical_and(sample_diff >= -eps, sample_diff <= eps)
    total_in_samples = torch.sum(torch.all(in_samples, dim=1))
    print("in samples", total_in_samples)
...
def compute_multi_dim_small_ball_probability_fixed_eps(Dyn_gp_X_train, Dyn_gp_Y_train, params, N_grid=8, prob_eps=None):
    ...
    assert not torch.any(torch.isnan(samples))
    sample_diff = samples.squeeze() - model_call.mean.squeeze()
    r_1 = torch.tensor([4.50618781e-01]).sqrt()/torch.tensor([4.28592943e-01]).sqrt()
    r_2 = torch.tensor([4.90922812e+00]).sqrt()/torch.tensor([4.28592943e-01]).sqrt()
    sample_diff[:,0,:]*=r_1
    sample_diff[:,2,:]*=r_2
    sample_diff = torch.abs(sample_diff.reshape(total_samples,-1))
    # plt.plot(X, samples.transpose(0, 1), lw=0.1)
    # plt.savefig("samples.png")
    max_val, _ = torch.max(sample_diff,1)
    eps = torch.quantile(max_val, prob_eps.cpu())
    # eps = params["agent"]["tight"]["dyn_eps"]
    # in_samples = torch.logical_and(sample_diff >= -eps, sample_diff <= eps)
    # total_in_samples = torch.sum(torch.all(in_samples, dim=1))
    # print("in samples", total_in_samples)
