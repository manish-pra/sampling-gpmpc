...
def tr(x):
    return np.exp(-(x+1.5)**2) + 1.8 * np.exp(-(x-1.5)**2) + np.cos(x*10)/10
...
plt.plot(x, tr(x), linewidth=2.5, label=r'$g^{tr}(x,a)$')
# plt.plot(x, tr(x)-0.08, linewidth=2.5)
plt.fill_between(x, f(x)-sig(x), f(x)+sig(x), alpha=0.5, label=r'$\left[l_{D}(x,a),u_D(x,a)\right]$',)
...
hi_0 = (nbar-4)*hi
hi_o = 0
rect1 = patches.Rectangle((-1.5, (nbar-3)*hi), 0.1, hi, linewidth=0.5,
                          edgecolor='None', facecolor='k', alpha=0.5, label='$X_0$')
...
rect4 = patches.Rectangle((up_cross[0], hi_p), down_cross[0] - up_cross[0], hi, linewidth=1,
                          edgecolor='None', facecolor='tab:green', alpha=0.5, label=r'$\mathcal{S}^{p}_n$')
...
rect5 = patches.Rectangle((up_cross[0], hi_o), down_cross[0] - up_cross[0], hi, linewidth=1,
                          edgecolor='None', facecolor='tab:orange', alpha=0.5, label=r'$\mathcal{S}^{o}_n$')
...
plt.legend(frameon=False, ncol=1, prop={'size': 24},columnspacing=0.6)
hide_spines(plt.gca())
hide_all_ticks(plt.gca())
plt.tight_layout(pad=0.01)
fname = Path().resolve().joinpath('figures')
fname.mkdir(exist_ok=True)
plt.savefig(str(fname.joinpath('confidence_bounds.pdf')),
