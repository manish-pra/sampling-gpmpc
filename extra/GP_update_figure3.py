...
model3_F_x_2 = torch.Tensor([[-0.5], [1.4], [0.4]]) - 3
model3_F_y_2 = torch.Tensor([[0.5], [2.2], [1]]) 

model3_F_x_3 = torch.Tensor([[0.25], [-0.5], [1]]) + 3
model3_F_y_3 = torch.Tensor([[1.4], [2.5], [1.2]]) - 1.4
...
Fx_X = torch.vstack([Fx_X_init, model3_F_x, model3_F_x_2])
Fx_Y = torch.vstack([Fx_Y_init, model3_F_y, model3_F_y_2])
# Fx_X = torch.vstack([Fx_X_init, model3_F_x])
# Fx_Y = torch.vstack([Fx_Y_init, model3_F_y])
...
def f(x):
    return Fx_model.posterior(torch.Tensor(x).reshape(-1,1)).mean.reshape(-1).detach().numpy()


def sig(x):
    std = Fx_model.posterior(torch.Tensor(x).reshape(-1,1)).variance.reshape(-1).detach()
...
tr_x = torch.Tensor([ 0.1470,  0.2023,  0.2446,  0.2989,  0.3366,  0.3775,  0.4124,  0.4420,
         0.4849,  0.5113,  0.5450,  0.5745,  0.5994,  0.6123,  0.6432,  0.6519,
         0.6728,  0.6858,  0.6898,  0.6965,  0.6939,  0.6915,  0.6910,  0.6814,
         0.6585,  0.6439,  0.6228,  0.6016,  0.5733,  0.5421,  0.5096,  0.4648,
         0.4315,  0.3875,  0.3396,  0.2935,  0.2442,  0.2042,  0.1632,  0.1073,
         0.0628,  0.0187, -0.0242, -0.0640, -0.1006, -0.1271, -0.1636, -0.1843,
        -0.2069, -0.2215, -0.2348, -0.2350, -0.2366, -0.2396, -0.2328, -0.2179,
        -0.2099, -0.1876, -0.1668, -0.1422, -0.1135, -0.0771, -0.0498, -0.0080,
         0.0308,  0.0626,  0.1087,  0.1515,  0.1959,  0.2420,  0.2840,  0.3387,
         0.3880,  0.4380,  0.4896,  0.5455,  0.5925,  0.6479,  0.7048,  0.7606,
         0.8107,  0.8736,  0.9301,  0.9843,  1.0375,  1.0922,  1.1405,  1.2057,
         1.2507,  1.2990,  1.3431,  1.3875,  1.4257,  1.4582,  1.4907,  1.5258,
         1.5500,  1.5660,  1.5860,  1.5970,  1.6050,  1.6107,  1.6111,  1.6033,
         1.6002,  1.5878,  1.5713,  1.5567,  1.5453,  1.5271,  1.5079,  1.4862,
         1.4654,  1.4425,  1.4262,  1.4134,  1.3872,  1.3742,  1.3614,  1.3496,
         1.3411,  1.3423,  1.3392,  1.3441,  1.3377,  1.3470,  1.3534,  1.3629,
         1.3803,  1.3855,  1.4034,  1.4169,  1.4490,  1.4623,  1.4767,  1.5012,
         1.5204,  1.5409,  1.5655,  1.5876,  1.6066,  1.6354,  1.6525,  1.6743,
         1.6918,  1.7141,  1.7311,  1.7463,  1.7673,  1.7930,  1.7996,  1.8232,
         1.8343,  1.8536,  1.8676,  1.8808,  1.8902,  1.8949,  1.9061,  1.9064,
         1.9073,  1.9145,  1.9153,  1.9155,  1.9080,  1.8985,  1.8955,  1.8707,
         1.8572,  1.8389,  1.8108,  1.7843,  1.7570,  1.7217,  1.6848,  1.6503,
         1.6056,  1.5591,  1.5078,  1.4527,  1.4065,  1.3437,  1.2957,  1.2450,
         1.1905,  1.1333,  1.0789,  1.0188,  0.9558,  0.8938,  0.8323,  0.7655,
         0.7005,  0.6297,  0.5717,  0.5030,  0.4427,  0.3816,  0.3194,  0.2625])
...
idx=1
# rect4 = patches.Rectangle((up_cross[idx], hi_p), down_cross[idx] - up_cross[idx], hi, linewidth=1,
#                           edgecolor='None', facecolor='tab:green', alpha=0.65, label=r'$\mathcal{R}_T(\mathbb{X}_n,\mathcal{S}^{p}_n)$')
...
rect1 = patches.Rectangle((st_loc - 0.1/2, hi_0), 0.1, hi, linewidth=0.5,
                          edgecolor='None', facecolor='k', alpha=0.5, label=r'$\mathbb{X}_n$')
...
rect6 = patches.Rectangle((x_left[1], hi_pl), x_right[1] - x_left[1], hi, linewidth=1,
                          edgecolor='None', facecolor='tab:orange', alpha=0.65, label=r'$\mathcal{R}_T(\mathbb{X}_n,\mathcal{S}^{{p\!}_L}_n)$')
...
up_cross, down_cross = get_cross(f(x)-sig(x), th=qx)
# idx=1
# rect3 = patches.Rectangle((x_left[1], hi_exp), -x_left[1] + up_cross[idx], hi, linewidth=1,
#                           edgecolor='None', facecolor='tab:purple', alpha=0.65, label=r'$\mathcal{G}_n(\mathbb{X}_n,\mathcal{S}^{{p\!}_L}_n)$')
# idx=1
# rect2 = patches.Rectangle((x_right[1], hi_exp), -x_right[1] + down_cross[idx], hi, linewidth=1,
#                           edgecolor='None', facecolor='tab:purple', alpha=0.65)
...
# plt.plot(model2_F_x, model2F_y, 'x', color='tab:orange', ms=14, mew=3)
plt.plot(model3_F_x, model3_F_y, 'x', color='tab:purple', ms=14, mew=3)

plt.plot(model3_F_x_2, model3_F_y_2, '+', color='purple',  ms=16, mew=3)
plt.plot(model3_F_x_3, model3_F_y_3, '*', color='purple',  ms=16, mew=3)

plt.legend(frameon=False, ncol=4, prop={'size': 20}, columnspacing=0.6, loc='lower left',bbox_to_anchor=(-0.02, -0.067))
hide_spines(plt.gca())
hide_all_ticks(plt.gca())
plt.tight_layout(pad=0.0)
fname = Path().resolve().joinpath('figures')
fname.mkdir(exist_ok=True)
plt.savefig(str(fname.joinpath('gp_orig_32.png')),
