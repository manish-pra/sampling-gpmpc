...
class DEMPC:
    ...
    def one_step_planner(self, st_curr):
        ...
        # set objective as per desired goal
        t_0 = timeit.default_timer()
        solver_status, cost, w, X_samples = self.dempc_solver.solve(self.agent, solver)
        t_1 = timeit.default_timer()
        dt = t_1 - t_0
        print("Time to solve", dt)
        ...
        if self.params["agent"]["shift_soln"]:
            ...
            X, U, Sl = self.dempc_solver.get_solution(solver)
        #
        self.visu.record(st_curr, X, U, dt, cost,solver_status, w, record_gp_model=False, X_samples=X_samples)
        print("X", X)
        print("U", U)
