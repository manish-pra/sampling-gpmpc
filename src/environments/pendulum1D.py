...
class Pendulum(object):
    ...
    def traj_initialize(self, x_curr):
        ...
        for _ in range(self.params["optimizer"]["H"]):
            u = -K @ (x - np.array(self.params["env"]["goal_state"]))  # LQR control law
            u = np.clip(
                u,
                self.params["optimizer"]["u_min"][0],
                self.params["optimizer"]["u_max"][0],
            ...
            xu = torch.from_numpy(np.hstack([x, u]))
            x = self.discrete_dyn(xu.reshape(1, -1)).numpy().reshape(-1)
            x_history.append(x.flatten())
            u_history.append(u.flatten())
        return np.vstack(x_history), np.vstack(u_history)
