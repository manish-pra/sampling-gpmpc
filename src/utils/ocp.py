...
def export_dempc_ocp(params):
    ...
    const_expr, p = dempc_const_expr(x_dim, n_order, params) # think of a pendulum and -pi/2,pi, pi/2 region is unsafe

    model = export_linear_model(name_prefix + 'dempc', p, params["agent"]["num_dyn_samples"])     # start pendulum at 0
...
def dempc_const_val(ocp, params, x_dim, n_order):
    ...
    x0 = np.array(params["env"]["start"]*params["agent"]["num_dyn_samples"]) #np.ones(x_dim)*0.72
